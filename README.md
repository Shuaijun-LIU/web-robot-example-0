...
These are two independently selectable implementations of the same four-arm assembly staging area. Both use a `0.90 m` Panda ring, slotted aluminum frame, loose cross-member, mounting plate, four fasteners, three tool stations, and handover pad. Assembly1 now gives each beam end one hollow circular interface and one hollow square interface, adds recessed dual-arm grasp pockets, keeps four lateral retention stops, and uses four matching round fasteners. Its scripted flow assigns Arm 2 the horizontal double-face hammer and ends Step 4 with a staged strike/recovery after the first fastener is placed. Assembly2 keeps the related task geometry while using converted RoboTwin tool meshes with palette-baked color regions and simple collision proxies.
...
- Go2 + Airbot: [UniLab](https://github.com/unilabsim/UniLab) (Apache-2.0; locally adapted actuator and root names)
