...
const screenshotPath = resolve('artifacts/screenshots/unitree-action-lab.png');
const failures = [];
...
try {
  ...
  await page.waitForFunction(
    () => Boolean(window.robotDemo?.runUnitreeAction && window.robotDemo?.getUnitreeActionState),
    null,
    { timeout },
  );
  ...
  await page.waitForFunction(
    () => document.documentElement.dataset.unitreeActionPhase === 'scan-wave'
      || document.documentElement.dataset.unitreeActionStatus === 'error',
    null,
    { timeout },
  );
  const scanState = await page.evaluate(() => window.robotDemo.getUnitreeActionState());
  if (scanState.status === 'error') throw new Error(`Unitree action failed: ${scanState.error}`);
  await page.waitForFunction(
    () => window.robotDemo.getUnitreeActionState().elapsed >= 3.8,
    null,
    { timeout },
  );
  const mid = await page.evaluate((names) => ({
    joints: window.robotDemo.getJointPositions(names),
  ...
  const g1WristDelta = jointDelta(home.joints, mid.joints, jointNames[0]);
  const go2LegDelta = jointDelta(home.joints, mid.joints, jointNames[1]);
  const airbotDelta = jointDelta(home.joints, mid.joints, jointNames[2]);
  if (g1WristDelta <= 0.2) throw new Error(`G1 wrist moved only ${g1WristDelta}`);
  if (go2LegDelta <= 0.1) throw new Error(`Go2 leg moved only ${go2LegDelta}`);
  ...
  console.log(JSON.stringify({
    ...
    midPhase: mid.state.phase,
    jointMotion: { g1WristDelta, go2LegDelta, airbotDelta },
    g1: { finalHeight: g1Height, finalTiltDegrees: g1Tilt, contacts: g1Contacts },
    go2: { finalHeight: go2Height, finalTiltDegrees: go2Tilt, contacts: go2Contacts },
