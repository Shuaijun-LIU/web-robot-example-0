...
test('PiPER moves the complete hammer station inward to clear its east arm base', () => {
  assert.deepEqual(PIPER_ASSEMBLY1_LAYOUT.taskStations.hammer, [0.57, 0, 0.229]);
  assert.match(layoutXml(PIPER_ASSEMBLY1_LAYOUT), /double_face_hammer" pos="0\.57 0 \.229"/);
  for (const name of [
    'tool_mat_hammer',
    ...
    assert.equal(object?.position[0], 0.57);
  }
