...
test('Assembly1 exposes one four-step sequence panel and deterministic Step 4 diagnostics', async () => {
  ...
  assert.match(panelSource, /Step 2: Lower and grasp/);
  assert.match(panelSource, /Step 1 complete/);
  assert.match(panelSource, /Step 2 complete: all four physical grasps verified/);
  assert.match(panelSource, /Step 3: Transfer and align/);
  assert.match(panelSource, /Step 3 complete: hammer staged, cross-member seated/);
  assert.match(panelSource, /Step 4: Insert fastener and tap/);
  assert.match(panelSource, /Arm 4 is tapping the fastener/);
  assert.match(panelSource, /Step 4 complete: fastener seated and tapped/);
  assert.match(panelSource, /Press Reset and try again/);
